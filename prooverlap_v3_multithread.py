...
parser.add_argument("--input", required = True, help="Input bed file, must contain 6 or 12 columns, name and score can be placeholder but score is required in --RankTest mode, strand is used only if some strandess test are requested")
parser.add_argument("--targets", required = True, help="Target bed file(s) (must contain 6 or 12 columns) to test enrichement against, if multiple files are supplied N independent test against each file are conducted, file names must be comma separated, the name of the file will be use as the name output")
parser.add_argument("--background", help="Background bed file (must contain 6 r 12 columns) to test enrichement aginst, should be a superset from wich input bed file is derived")
...
parser.add_argument("--exclude_intervals", default = None, help="Path to a BED file. Exclude regions overlapping with regions in the supplied BED file")
...
parser.add_argument("--RankTest", action = "store_true", help="Activates the Running Rank analyis")
...
parser.add_argument("--alpha", default = "0.5", type=float, help="Relative Influence of the overlap fraction/distance (with respect to ranking) weightRanked test, only if --WeightRanking is active, must be between 0 and 1")
