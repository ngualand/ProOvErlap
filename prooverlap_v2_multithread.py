...
# Suppress some warnings, i.e Not print to stout
...
parser.add_argument("--outdir", help="Full path to output directory to store tables for plot, it will be created")
...
parser.add_argument("--alpha", default = "0.5", type=float, help="Relative Influence of the overlap fraction (with respect to ranking)or abslute distance in weightRanked test, only if --WeightRanking is active")
parser.add_argument("--w", default = "0.25", type=float, help="Strength of the Weight for the ranking test, only if --WeightRanking is active")
...
def compute_length_random_bg(input_bed_file, exclude_intervals, genome):
    ...
    # Read the exclude regions, if provided
    exclude_intervals_bed = read_exclude_bed_file(exclude_intervals) if exclude_intervals else None
    
    # Generate the same number of random intervals as the input file
    num_random_intervals = len(existing_intervals)
    random_intervals = pd.DataFrame(generate_random_intervals_frequency(existing_intervals, mean_length, chrom_count, num_random_intervals, exclude_intervals_bed))
    
    
    # Compute the nucleotide content
...
def compute_gc_at_content_random_bg(input_bed_file, exclude_intervals, genome):
    ...
    # Return the mean GC and AT content
...
def test_GC_AT(bed,background,randomization, target, frame, table, genome, exclude_intervals = None, thread = 1):
    ...
    # Unpacking GC and AT values into separate lists
    random_count_GC, random_count_AT = zip(*results)
...
#def weighted_score(x, y, alpha=0.1, ascending = True):
#                N = len(x) 
#                if ascending == True:
...
#                else:
#                    rank_normalized = (np.argsort(-x) + 1) / N
#                    return [xi * (1 - alpha * (rank / yi)) for xi, yi, rank in zip(x, y, rank_normalized)]
...
def compute_shuffle(i, y, cumulative_real):
    y_shuffled = np.random.permutation(y)  # Shuffle the labels
    cumulative_shuffle = np.cumsum(y_shuffled) / np.sum(y_shuffled)

    # KS test between the real and shuffled distributions
...
def parallel_shuffling(n_shuffles, y, cumulative_real, thread):
    ...
    with Pool(processes=thread) as pool:
        # Create a list of inputs for each shuffle
        inputs = [(i, y, cumulative_real) for i in range(n_shuffles)]
        
        # Use starmap to execute the function in parallel
...
def test_closest_rank(bed,target,ov_fraction, randomization, strandness="strandless", exclude_intervals = None, exclude_ov = False, exclude_upstream = False, exclude_downstream=False, ascending = False, WeightRanking = False, alpha = 0.5, w = 0.25, thread = 1):
    ...
    #This is neede because we want to support closest element no far element so we create the specular of scaled distances
    ...
    forplot = pd.DataFrame({"Peak_idx" : peak_idx, "ES": ES, "Pval" : Pval, "cumulative_real": "|".join(map(str, cumulative_real)) , "mean_shuffle_cdf": "|".join(map(str, mean_shuffle_cdf))}, index = [0])
