...
# How to plot results?
ProOvErlap supports the creation of two main types of graphical outputs (although you may also perform your own plots, as all data are saved to files). The first one is a density plot (generated by the Plot.R script), which shows how far the obtained results deviate from what would be expected by chance. Usage: Rscript Plot.R --help. Moreover, ProOvErlap also creates heatmaps of the Z-score for each target and, optionally, genomic regions or custom regions, using the Heatmap.R script. Usage: Rscript Heatmap.R --help.
